        ...
        function animationLoop() {
          if (Math.abs(_offset - offset) < 0.5) {
            this.animating = false;
						_offset = offset;
						// 设置 element 的 webkitTransform
            element.style.webkitTransform = '';
            if (nextElement) {
              nextElement.style.webkitTransform = '';
            }
						cancelAnimationFrame(raf);
						// 如果 callback 函数存在, 执行 callback 函数回调
            if (callback) {
              callback();
            }
            ...
            return;
					...
					// 将计算之后的 _offset 进行传递给 translate3d
          ...
          if (nextElement) {
            nextElement.style.webkitTransform = `translate3d(${_offset - offset}px, 0, 0)`;
					...
					// requestAmimationFrame 是什么东西
          raf = requestAnimationFrame(animationLoop.bind(this));
				}
				// 执行绑定使用 animationLoop.call(this);
          ...
          var transitionEndCallback = () => {
            ...
            if (callback) {
              callback.apply(this, arguments);
					...
					// 执行的 once 函数, 当 element 的 webkieTransitionEnd 被触发的时候，执行 transitionEndCallback 函数
