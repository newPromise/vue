...
let result = lyrics.match(pattern);
let sd = [];
let s;
// 转化为秒
...
// 转化为二维数组 [[时间:歌词]] 的形式
for (let a of result) {
  let startIndex = lyrics.indexOf(a);
  let endIndex = lyrics.indexOf(result[result.indexOf(a) + 1]);
  s = lyrics.slice(startIndex, endIndex);
  let time = s.slice(s.indexOf('[') + 1, s.indexOf(']'));

  let value = s.replace(pattern, '');
  sd.push([toSecondes(time), value]);
}
// 当存在两个时间的时候
sd.map((item, index) => {
  if (!item[1]) {
